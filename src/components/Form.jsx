...
const Form = () => {
    ...
    return (
      <form ref={form} onSubmit={sendEmail} className='text-black flex flex-col w-3/4 mb-10 max-w-[600px] md:text-lg'>
        ...
        <input type="text" name="user_name" className='bg-white p-2 mb-5' placeholder='Twoje imię' onChange={(e) => {handleInputsChange(setName, e)}} value={name}/>
        <label className='text-white text-lg md:text-xl'>Email</label>
        <input type="email" name="user_email" className='bg-white p-2 mb-5' placeholder='Twój email' onChange={(e) => {handleInputsChange(setEmail, e)}} value={email}/>
